<?php 
    ...
    $listarPermisosUsuariosOperativo = $operativo->listarPermisosUsuariosOperativo();
                ...
                <table id="dataT" class="table table-hover table-sm display text-center" style="width:100%">
                    <thead style="background-color: #1b2d3b; color: #fff;">
                        <tr>
                            ...
                            <th style="vertical-align: top;">ROL</th>
                            <th style="vertical-align: top;">TIPO USUARIO</th>
                    ...
                    <tbody>
                        <?php foreach ($listarPermisosUsuariosOperativo as $lpuo){ 
                            
                            $listarUsuarioPorId = $usuario->listarUsuarioPorId($lpuo['id_usuario']);
                            $listarUsuarioAsignadorPorId = $usuario->listarUsuarioPorId($lpuo['id_usuario_asignacion']); ?>

                            <tr>
                                <td><?php echo str_pad($lpuo['id'], 5, "0", STR_PAD_LEFT) ?></td>
                                <td><?php echo $listarUsuarioPorId[0]['nombre'] ?></td>
                                <td><?php echo $lpuo['rol']; ?></td>
                                <td><?php echo $lpuo['tipo_usuario']; ?></td>
                                <td><?php echo $lpuo['fecha_asignacion']; ?></td>
                                <td><?php echo $listarUsuarioAsignadorPorId[0]['nombre'] ?></td>
                                <td><i style="color:green; cursor:pointer;" class="fa fa-eye"></i></td>
                                <td>
                                    <a href="actualizarPermisosUsuariosOperativo.php?id=<?php echo $lpuo['id']; ?>" class="btn btn-outline-info" style="margin: 2px; border-radius: 4px; padding: 0px 2px 0px 2px;"><i class="fa fa-edit"></i></a>
                                </td>
                            </tr>
