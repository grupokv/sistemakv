...
$hoy = date('Ymd_His');
$filename = $hoy.'_reporte_vulnerabilidad.xls';
          ...
          <tbody>
            <?php foreach ($listarC as $lc){ ?>
				<tr>
					...
					<td><?php echo $datos_usu[0]['nombre']; ?></td>
					...
					<?php 
						if($lc['contrato'] == 0){
							...
							$det_contrato = $lc['contrato'].' - '.$datos_cliente[0]['nombre'];
						}
						echo $det_contrato;
					?>
					</td>
...
<td><?php echo $lc['otra_enfermedad'];?></td>
<td><?php echo $lc['cual'];?></td>
					<td><?php echo $lc['ninguna'];?></td>
<td><?php echo $lc['embarazo'];?></td>
<td><?php echo $lc['obesidad'];?></td>
