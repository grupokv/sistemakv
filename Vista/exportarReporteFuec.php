<?php
$hoy = date('Ymd_His');
$filename = $hoy.'_reporte_fuec.xls';
...
$filtrarFuecsOcasionales = $fuec->filtrarFuecs($id_usuario_emisor, $id_vehiculo, $fecha_inicial, $fecha_final);
?>
                                ...
                                <tbody>
                                    <?php foreach ($filtrarFuecsOcasionales as $lffo){ ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php 
                                                $listarContratosId = $contrato->listarId($lffo['id_contrato']);
                                                    foreach ($listarContratosId as $lci) {
                                                        $emp = $empresa->listarPorId($lci['id_empresa']);
                                                        $cli = $cliente->listarClientePorId($lci['id_cliente']);
                                                        echo "No. interno ". $lci['id_contrato'] . " Entre " . $cli[0]['razon_social'] . " y " . $emp[0]['nombre_empresa']; 
                                                    }
