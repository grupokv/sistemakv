...
if(!isset($_POST['id_usuario'])){
	...
	$query = "SELECT * FROM detalle_servicio where id_usuario = '$id'";
	...
	if($format == 'json') {
		...
		if($cant > 0){
			for($i=0;$i<count($posts);$i++){
		    	...
		    	$query1 = "SELECT * FROM frecuencia_servicio where id_servicio = '$ids'";
				...
				$per = $titulo->addChild('periodicidad',$periodicidad);
			}
			print($xml->asXML());
		} else {
			header('Content-type:text/xml');
		    ...
		    $Lname = $titulo->addChild('detalle','No se encontraron servicios para el usuario');
