...
if(!isset($_POST['id_solicitud'])){
	...
	$query = "SELECT * FROM solicitud_cliente where id_solicitud = '$id'";
	...
	if($format == 'json') {
		...
		if($cant > 0){
			for($i=0;$i<count($posts);$i++){
				...
				$id = $titulo->addChild('id',$posts[$i]['post']['id_solicitud']);
		    	$idc = $titulo->addChild('cant_idas',$posts[$i]['post']['servicios_ida']);
		    	$descripcion = $titulo->addChild('cant_retornos',$posts[$i]['post']['servicios_retorno']);
		    	$direccion = $titulo->addChild('id_cliente',$posts[$i]['post']['id_cliente']);
		    	$telefono = $titulo->addChild('id_solicitante',$posts[$i]['post']['id_usuario']);
		    	$encargado = $titulo->addChild('fecha',$posts[$i]['post']['fecha_solicitud']);
			}
			print($xml->asXML());
		} else {
			header('Content-type:text/xml');
		    ...
		    $Lname = $titulo->addChild('detalle','No se encontro solicitud');
