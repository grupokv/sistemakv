...
if(!isset($_POST['id_tipo_servicio'])){
	...
	$query = "SELECT * FROM tipo_servicio where id_tipo_servicio = '$id'";
	...
	if($format == 'json') {
		...
		header('Content-type:text/xml');
		$xml = new SimpleXMLElement("<?xml version='1.0' encoding='UTF-8'?><ArrayOfTipoServicio></ArrayOfTipoServicio>");
		//print_r($posts[1]['post']['id_tipo_vehiculo']);
		$cant = count($posts);
		if($cant > 0){
			for($i=0;$i<count($posts);$i++){
				$titulo = $xml->addChild('TipoServicio');
				$id = $titulo->addChild('id',$posts[$i]['post']['id_tipo_servicio']);
		    	$idc = $titulo->addChild('detalle',$posts[$i]['post']['nombre_tipo_servicio']);
			}
			print($xml->asXML());
		} else {
			header('Content-type:text/xml');
		    ...
		    $Lname = $titulo->addChild('detalle','No se encontro tipo de servicio');
