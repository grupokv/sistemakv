...
if(!isset($_POST['id_tipo_vehiculo'])){
    ...
    $query = "SELECT * FROM vehiculo where id_tipo_vehiculo = '$id'";
    ...
    if($format == 'json') {
        ...
        if($cant > 0){
            for($i=0;$i<count($posts);$i++){
                ...
                $id = $titulo->addChild('id',$posts[$i]['post']['id_vehiculo']);
                $perfil = $titulo->addChild('id_tipo_vehiculo',$posts[$i]['post']['id_tipo_vehiculo']);
                $nombre = $titulo->addChild('placa',$posts[$i]['post']['placa']);
                $empresa = $titulo->addChild('numero_movil',$posts[$i]['post']['numero_movil']);
            }
            print($xml->asXML());
        } else {
            header('Content-type:text/xml');
