...
if(!isset($_POST['id_pais'])){
	...
	if($format == 'json') {
		...
		header('Content-type:text/xml');
		$xml = new SimpleXMLElement("<?xml version='1.0' encoding='UTF-8'?><ArrayOfDatosDepartamento></ArrayOfDatosDepartamento>");
		//print_r($posts[1]['post']['id_tipo_vehiculo']);
		$cant = count($posts);
		if($cant > 0){
			for($i=0;$i<count($posts);$i++){
				$titulo = $xml->addChild('DatosDepartamento');
				$id = $titulo->addChild('id',$posts[$i]['post']['id_departamento']);
		    	$idc = $titulo->addChild('nombre',$posts[$i]['post']['departamento']);
			}
			print($xml->asXML());
		} else {
			header('Content-type:text/xml');
		    ...
		    $Lname = $titulo->addChild('detalle','No se encontraron departamentos para el pais');
