...
if($format == 'json') {
	...
	$xml = new SimpleXMLElement("<?xml version='1.0' encoding='UTF-8' standalone='yes'?><ArrayOfPais></ArrayOfPais>");
	//print_r($posts[1]['post']['id_tipo_vehiculo']);
	//echo count($posts);
	for($i=0;$i<count($posts);$i++){
		$titulo = $xml->addChild('Pais');
    	$name = $titulo->addChild('id',$posts[$i]['post']['id_pais']);
    	$Lname = $titulo->addChild('nombre',$posts[$i]['post']['pais']);
	}
