...
if($format == 'json') {
	...
	$xml = new SimpleXMLElement("<?xml version='1.0' encoding='UTF-8' standalone='yes'?><ArrayOfCliente></ArrayOfCliente>");
	//print_r($posts[1]['post']['id_tipo_vehiculo']);
	//echo count($posts);
	for($i=0;$i<count($posts);$i++){
		$titulo = $xml->addChild('Cliente');
    	$name = $titulo->addChild('id',$posts[$i]['post']['id_cliente']);
    	$Lname = $titulo->addChild('nombre',$posts[$i]['post']['razon_social']);
    	$nit = $titulo->addChild('nit',$posts[$i]['post']['nit_cliente']);
    	$dir = $titulo->addChild('direccion',$posts[$i]['post']['direccionC']);
