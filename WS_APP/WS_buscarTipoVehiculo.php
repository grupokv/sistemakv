...
if(!isset($_POST['id_tipo_vehiculo'])){
	...
	$query = "SELECT * FROM tipo_vehiculo where id_tipo_vehiculo = '$id'";
	...
	if($format == 'json') {
		...
		header('Content-type:text/xml');
		$xml = new SimpleXMLElement("<?xml version='1.0' encoding='UTF-8'?><ArrayOfTipoVehiculo></ArrayOfTipoVehiculo>");
		//print_r($posts[1]['post']['id_tipo_vehiculo']);
		$cant = count($posts);
		if($cant > 0){
			for($i=0;$i<count($posts);$i++){
				$titulo = $xml->addChild('TipoVehiculo');
				$id = $titulo->addChild('id',$posts[$i]['post']['id_tipo_vehiculo']);
		    	$idc = $titulo->addChild('detalle',$posts[$i]['post']['nombre_tipo_vehiculo']);
			}
			print($xml->asXML());
		} else {
			header('Content-type:text/xml');
		    ...
		    $Lname = $titulo->addChild('detalle','No se encontro tipo de vehiculo');
