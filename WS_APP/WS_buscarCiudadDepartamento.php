...
if(!isset($_POST['id_departamento'])){
	...
	$query = "SELECT * FROM ciudad where id_departamento = '$id'";
	...
	if($format == 'json') {
		...
		header('Content-type:text/xml');
		$xml = new SimpleXMLElement("<?xml version='1.0' encoding='UTF-8'?><ArrayOfDatosCiudad></ArrayOfDatosCiudad>");
		//print_r($posts[1]['post']['id_tipo_vehiculo']);
		$cant = count($posts);
		if($cant > 0){
			for($i=0;$i<count($posts);$i++){
				$titulo = $xml->addChild('DatosCiudad');
				$id = $titulo->addChild('id',$posts[$i]['post']['id_ciudad']);
		    	$idc = $titulo->addChild('nombre',$posts[$i]['post']['ciudad']);
			}
			print($xml->asXML());
		} else {
			header('Content-type:text/xml');
		    ...
		    $Lname = $titulo->addChild('detalle','No se encontraron ciudades para el departamento');
