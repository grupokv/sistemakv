...
if(!isset($_POST['id_tipo_novedad'])){
	...
	$query = "SELECT * FROM tipo_novedad where id_tipo_novedad = '$id'";
	...
	if($format == 'json') {
		...
		header('Content-type:text/xml');
		$xml = new SimpleXMLElement("<?xml version='1.0' encoding='UTF-8'?><ArrayOfTipoNovedad></ArrayOfTipoNovedad>");
		//print_r($posts[1]['post']['id_tipo_vehiculo']);
		$cant = count($posts);
		if($cant > 0){
			for($i=0;$i<count($posts);$i++){
				$titulo = $xml->addChild('TipoNovedad');
				$id = $titulo->addChild('id',$posts[$i]['post']['id_tipo_novedad']);
		    	$idc = $titulo->addChild('descripcion',$posts[$i]['post']['descripcion']);
			}
			print($xml->asXML());
		} else {
			header('Content-type:text/xml');
		    ...
		    $Lname = $titulo->addChild('detalle','No se encontro tipo de novedad');
