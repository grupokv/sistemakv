...
if(!isset($_POST['id_proyecto'])){
	...
	$query = "SELECT * FROM unidad_operativa where id_proyecto = '$id'";
	...
	if($format == 'json') {
		...
		header('Content-type:text/xml');
		$xml = new SimpleXMLElement("<?xml version='1.0' encoding='UTF-8'?><ArrayOfDatosUnidadOperativa></ArrayOfDatosUnidadOperativa>");
		//print_r($posts[1]['post']['id_tipo_vehiculo']);
		$cant = count($posts);
		if($cant > 0){
			for($i=0;$i<count($posts);$i++){
				$titulo = $xml->addChild('DatosUnidadOperativa');
				$id = $titulo->addChild('id',$posts[$i]['post']['id_unidad']);
		    	...
		    	$encargado = $titulo->addChild('id_ciudad',$posts[$i]['post']['id_ciudad']);
			}
			print($xml->asXML());
		} else {
			header('Content-type:text/xml');
		    ...
		    $Lname = $titulo->addChild('detalle','No se encontraron unidades operativas para el proyecto');
