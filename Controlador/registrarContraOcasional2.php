...
if ($_POST['familiar'] == 'S') {
	$fecha_final_contrato_ocasional = $_POST['fecha_final_familiar'];
}else{
...
$registrarCO = $contratoOcasional->regitrar($objeto_contrato, $id_empresa, $id_cliente, $id_vehiculo, $origen, $destino, $id_ciudad, $fecha_inicial_contrato_ocasional, $fecha_final_contrato_ocasional, $fecha_creacion, $hora_creacion, $valor_contrato, $id_responsable);
...
$cantidad = $fuec->listarFuecPorCliente($registrarCO,date('Y'));
...
$num_comprobante = $cod_ciudad.$num_aprobacion.$year_aprobacion.$year.$num_contrato.$cantidad;


$registrarFuecO = $fuec->registrarFuec($num_interno, $num_comprobante, $listarCiudadOrigen[0]['ciudad'], $listarCiudadDestino[0]['ciudad'], $tipo_fuec, $con_fuec, $id_vehiculo, $id_contrato, $id_co, $listarClientePorId[0]['razon_social'], $listarClientePorId[0]['nit_cliente'], $listarClientePorId[0]['direccionC'], $listarClientePorId[0]['telefonoC'], $id_responsable, date('Y-m-d H:i:s'), $anexo);
