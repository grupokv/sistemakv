...
$registrarProductoCliente = $operativo->registrarProductoCliente($id_cliente, $detalle_producto, $tipo_producto, $dias_aplicados_mensualidad, $dia_corte, $observaciones, $fecha_creacion, $id_usuario_creacion);

$id_producto = $registrarProductoCliente;
		...
		for ($i=0; $i < count($_POST['id_tipo_vehiculo']) ; $i++) {
			if ($_POST['id_tipo_vehiculo'][$i] == $ltv['id']) {
				...
				/* ---------- COSTO POR DISPONIBILIDAD ----------*/
					$cpd_cliente = $_POST['cpd_cliente' . strtolower($ltv['id'])];
					$cpd_movil = $_POST['cpd_movil' . strtolower($ltv['id'])];

					if (($cpd_cliente != '') || ($cpd_movil != '')) {
						$costo_por_disponibilidad = array('cliente' => $cpd_cliente, 'movil' => $cpd_movil);
						$costo_diponibilidad_json = json_encode($costo_por_disponibilidad);
					}else{
						$costo_diponibilidad_json = NULL;
					}


				$registrarTarifasProductoCliente = $operativo->registrarTarifasProductoCliente($id_tipo_vehiculo, $id_producto, $id_cliente, $valor_recorrido_json, $valor_hora_json, $recorridos_x_dia_json, $valor_mensual_json, $valor_relevo_sencillo_json, $valor_relevo_doble_json, $costo_diponibilidad_json);
...
echo "<script>alert('Se ha registrado correctamente la información.'); window.location.href = '../Vista/productosClientes.php';</script>";
