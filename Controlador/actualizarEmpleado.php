...
$eps = mb_strtoupper($_POST['eps']);
$arl = mb_strtoupper($_POST['arl']);
$pension = mb_strtoupper($_POST['pension']);
$cesantias = mb_strtoupper($_POST['cesantias']);
$caja_compensacion = mb_strtoupper($_POST['caja_compensacion']);
...
$act_contrato = $_POST['act_contrato'];
...
$act_actualizacion_datos = $_POST['act_actualizacion_datos'];
$fecha_actualizacion = $_POST['fecha_actualizacion'];
if($fecha_actualizacion == ''){
	$fecha_actualizacion = '0000-00-00';
}
$induccion = $_FILES['induccion']['name'];
$act_induccion = $_POST['act_induccion'];
$fecha_induccion = $_POST['fecha_induccion'];
if($fecha_induccion == ''){
	$fecha_induccion = '0000-00-00';
}
$evaluacion = $_FILES['evaluacion']['name'];
$act_evaluacion = $_POST['act_evaluacion'];
$fecha_evaluacion = $_POST['fecha_evaluacion'];
if($fecha_evaluacion == ''){
	$fecha_evaluacion = '0000-00-00';
}
$manual_funciones = $_FILES['manual_funciones']['name'];
$act_manual_funciones = $_POST['act_manual_funciones'];

$afiliacion_eps = $_FILES['afiliacion_eps']['name'];
$act_afiliacion_eps = $_POST['act_afiliacion_eps'];
$afiliacion_arl = $_FILES['afiliacion_arl']['name'];
$act_afiliacion_arl = $_POST['act_afiliacion_arl'];
$afiliacion_caja = $_FILES['afiliacion_caja']['name'];
$act_afiliacion_caja = $_POST['act_afiliacion_caja'];

$cant_otros = $_POST['cant_otros'];
$nombre_archivo = $_POST['nombre_archivo'];
$archivos = $_FILES['archivos'];
$act_archivos = $_POST['act_archivos'];
...
$act_certificado_exp = $_POST['act_certificado_exp'];
...
$act_certificado_cur = $_POST['act_certificado_cur'];

if(($nombres != '')and($apellidos != '')and($num_documento != '')){

	$buscar = $empleado->buscarEmpleadoPorCedula($num_documento);
	if(count($buscar) > 0){
		...
		if (!empty($_FILES['fotocopia_documento']['name'])) {                
			...
			$fotocopia_documento = $act_fotocopia_documento;
		}
		...
		if (!empty($_FILES['actualizacion_datos']['name'])) {                
			...
			$actualizacion_datos = $act_actualizacion_datos;
		}
		
		if (!empty($_FILES['induccion']['name'])) {                
			$carpeta = '../Documentos/Empleados/'. $num_documento;
			$ruta = $carpeta .'/'. $fecha.'-'.  $_FILES['induccion']['name'];
			$induccion = $fecha .'-'. $induccion;
			if (!file_exists($carpeta)) {
				mkdir($carpeta, 0757, true);
			}
			$ruta_temp = $_FILES['induccion']['tmp_name'];
			move_uploaded_file($ruta_temp, $ruta);
		} else {
			$induccion = $act_induccion;
		}
		
		if (!empty($_FILES['evaluacion']['name'])) {                
			$carpeta = '../Documentos/Empleados/'. $num_documento;
			$ruta = $carpeta .'/'. $fecha.'-'.  $_FILES['evaluacion']['name'];
			$evaluacion = $fecha .'-'. $evaluacion;
			if (!file_exists($carpeta)) {
				mkdir($carpeta, 0757, true);
			}
			$ruta_temp = $_FILES['evaluacion']['tmp_name'];
			move_uploaded_file($ruta_temp, $ruta);
		} else {
			$evaluacion = $act_evaluacion;
		}
		...
		if (!empty($_FILES['manual_funciones']['name'])) {                
			...
			$manual_funciones = $act_manual_funciones;
		}
		...
		if (!empty($_FILES['afiliacion_eps']['name'])) {                
			...
			$afiliacion_eps = $act_afiliacion_eps;
		}
		...
		if (!empty($_FILES['afiliacion_arl']['name'])) {                
			...
			$afiliacion_arl = $act_afiliacion_arl;
		}
		...
		if (!empty($_FILES['afiliacion_caja']['name'])) {                
			...
			$afiliacion_caja = $act_afiliacion_caja;
		}
			...
			for($i=0;$i<$cant_estudios;$i++){
				if(($titulo[$i] != '')and($universidad[$i] != '')and($nivel[$i] != '')){
					...
					if (!empty($diploma['name'][$i])) {                
						...
						if(isset($act_diploma[$i])){
							$archivo = $act_diploma[$i];
						} else {
							$archivo = '';
						}
					}
					$registrarEstudio = $empleado->registrarEstudio($titulo[$i], $universidad[$i], $nivel[$i], $fec_grado, $archivo, $id);
				}
			}
		}
		
		if($cant_experiencias > 0){
			$borrar = $empleado->borrarExperienciasEmpleado($id);
			for($i=0;$i<$cant_experiencias;$i++){
				if(($empresa_exp[$i] != '')and($cargo_exp[$i] != '')and($telefono_exp[$i] != '')){
					...
					if (!empty($certificado_exp['name'][$i])) {                
						...
						if(isset($act_certificado_exp[$i])){
							$archivo = $act_certificado_exp[$i];
						} else {
							$archivo = '';
						}
					}
					$registrarExperiencia = $empleado->registrarExperiencia($empresa_exp[$i], $fechai, $fechaf, $cargo_exp[$i], $telefono_exp[$i], $jefe_exp[$i], $archivo, $id);
				}
			}
		}
		
		if($cant_cursos > 0){
			$borrar = $empleado->borrarCursosEmpleado($id);
			for($i=0;$i<$cant_cursos;$i++){
				if(($nombre_curso[$i] != '')and($institucion[$i] != '')){
					...
					if (!empty($certificado_cur['name'][$i])) {                
						...
						if(isset($act_certificado_cur[$i])){
							$archivo = $act_certificado_cur[$i];
						} else {
							$archivo = '';
						}
					}
					$registrarCurso = $empleado->registrarCurso($nombre_curso[$i], $intensidad[$i], $institucion[$i], $archivo, $fec_curso, $id);
				}
			}
		}
		...
		if($cant_otros > 0){
			$borrar = $empleado->borrarDocumentosEmpleado($id);
			for($i=0;$i<$cant_otros;$i++){
				if($nombre_archivo[$i] != ''){
					if (!empty($archivos['name'][$i])) {                
						...
						if(isset($act_archivos[$i])){
							$archivo = $act_archivos[$i];
						} else {
							$archivo = '';
						}
					}
					$registrarCurso = $empleado->registrarDocumento($nombre_archivo[$i], $archivo, $id);
				}
			}
		}
		
		echo ("<script LANGUAGE='JavaScript'>
		window.alert('El empleado fue editado correctamente');
		window.location.href='../Vista/empleados.php';
		</script>");
		
	} else {
		echo ("<script LANGUAGE='JavaScript'>
		window.alert('Error al intentar el proceso');
		window.location.href='../Vista/empleados.php';
		</script>");
	}
